...
@Component({
  selector: 'app-rzsq',
  templateUrl: './rzsq.component.html',
  styles: [`
      :host [nz-form] {
          margin: 20px auto 0;
          max-width: 500px;
...
export class RzsqComponent implements OnInit {
    ...
    toCreateForm(item) {
        ...
        this.router.navigate(['/lvl/rz/yfsq'], { queryParams: {
                psn: this.currentPsn,
    ...
    _submitForm() {
        ...
        this.loading = true;
        this.lvService.saveYfsq(params, bpm)
            .pipe(
                tap(() => this.loading = false)
            )
            .subscribe(res => {
    ...
    ngOnInit() {
        this.uuid = uuid();
        this.fileInfo.postFileData = {attachfiletype: 'prepaid', attachfileuuid: this.uuid};
        // 获取融资项目列表
        this.lvService.getRzProjects({
    ...
    stepBack() {
        this.current_step = 1;
        this.location.replaceState('/lvl/rz/yfsq');
    }
